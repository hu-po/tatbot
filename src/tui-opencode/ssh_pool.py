...
class SSHPool:
    def __init__(self, config: Optional[SSHConfig] = None):
        ...
        if config is None:
            ...
            conn_to_env = os.environ.get("TATBOT_TUI_SSH_CONNECT_TIMEOUT")
            config = SSHConfig(
                key_path=key_env,
                port=int(port_env) if port_env else None,
                timeout=float(conn_to_env) if conn_to_env else 5.0,
            )
        self.config = config
    ...
    def _connect(self, host: str, user: str, port: int) -> paramiko.SSHClient:
        ...
        try:
            banner_timeout = float(os.environ.get("TATBOT_TUI_SSH_BANNER_TIMEOUT", "3"))
            client.connect(
                hostname=host,
                port=port,
                username=user,
