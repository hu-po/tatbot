...
def get_available_nodes(config_root: Optional[Path] = None) -> List[str]:
    """Get list of all available node names from nodes.yaml configuration.
    ...
    return [node["name"] for node in config.get("nodes", [])]
    ...
    if len(sys.argv) < 2:
        ...
        if not node:
            try:
                ...
                print(f"Available nodes: {', '.join(available_nodes)}", file=sys.stderr)
            except FileNotFoundError:
                print("Usage: python -m tatbot.utils.node_config <node_name>", file=sys.stderr)
