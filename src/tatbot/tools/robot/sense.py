...
async def sense_tool(input_data: SenseInput, ctx: ToolContext):
    ...
    try:
        ...
        if getattr(input_data, "calibrate_extrinsics", True):
            ...
            if image_paths:
                try:
                    ...
                    compare_extrinsics_with_urdf(calibrated_cams, scene, max_deviation)
        ...
        link_poses = get_link_poses(scene.urdf.path, scene.urdf.cam_link_names, scene.ready_pos_full.joints)
        ...
        for ply_idx in range(num_plys):
            ...
            for _, depth_cam in depth_cameras.items():
                expected_ply_path = (
                    Path(depth_cam.save_dir) / f"{depth_cam.save_prefix}{depth_cam.frame_idx:06d}.ply"
                )
        ...
        try:
            ...
            for cam in cams_to_save.ipcameras:
                ...
                K = np.array(
                    [
                        [cam.intrinsics.fx, 0, cam.intrinsics.ppx],
        ...
        if getattr(input_data, "enable_vggt", False):
            try:
                ...
                try:
                    ...
                    preferred_node = (
                        cfg_dict.get("cam", {}).get("vggt", {}).get("preferred_gpu_node", preferred_node)
                    )
        ...
        log.error(error_msg)
        yield SenseOutput(success=False, message=error_msg, captured_files=captured_files)
