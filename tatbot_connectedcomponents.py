...
class TatbotConfig:
    ...
    desired_batch_size_px: int = 100
    """Number of pixel-targets per batch (approx.)."""
...
def split_blob_grid(xs: np.ndarray, ys: np.ndarray, B: int):
    """
    Given arrays xs, ys of length N_blob, sort them in lexicographic (x, then y)
    order and split into chunks of size ≤ B. Returns a list of (xs_chunk, ys_chunk).
    """
    N_blob = xs.shape[0]
    if N_blob == 0:
        return []
    ...
    ys_sorted = ys[order]
    subbatches = []
    for start in range(0, N_blob, B):
        end = min(start + B, N_blob)
        xs_chunk = xs_sorted[start:end]
        ys_chunk = ys_sorted[start:end]
        subbatches.append((xs_chunk, ys_chunk))
    return subbatches

def merge_small_blobs(pool: list[tuple[np.ndarray, np.ndarray]], MIN_BATCH: int):
    """
    pool: list of (xs, ys) each with size < MIN_BATCH.
    Merge nearest-centroid pairs until each merged cluster ≥ MIN_BATCH,
    or until only one undersized cluster remains. Returns list of (xs, ys).
    """
    merged = []
    working = []
    for xs_s, ys_s in pool:
        cx = xs_s.mean()
        cy = ys_s.mean()
        working.append({'xs': xs_s, 'ys': ys_s, 'centroid': (cx, cy), 'N': xs_s.shape[0]})
    while working:
        A = working.pop(0)
        if A['N'] >= MIN_BATCH:
            merged.append((A['xs'], A['ys']))
            continue
        best_i = None
        best_d = float('inf')
        xA, yA = A['centroid']
        ...
        if best_i is None:
            merged.append((A['xs'], A['ys']))
            break
        B = working.pop(best_i)
        xs_merged = np.concatenate([A['xs'], B['xs']])
        ys_merged = np.concatenate([A['ys'], B['ys']])
        N_merged = xs_merged.shape[0]
        cx_m = xs_merged.mean()
        cy_m = ys_merged.mean()
        if N_merged >= MIN_BATCH:
            merged.append((xs_merged, ys_merged))
        else:
            working.append({'xs': xs_merged, 'ys': ys_merged,
                            'centroid': (cx_m, cy_m), 'N': N_merged})
    return merged
...
def main(config: TatbotConfig):
    ...
    log.info("🔢 Creating pixel batches via connected‐components (OpenCV)…")
    # 1) Build binary mask: 1 where "ink" pixels exist, 0 elsewhere
    binary_mask = (img_np <= config.image_threshold).astype(np.uint8)
    # 2) Run connectedComponents to label each contiguous blob
    num_labels, labels = cv2.connectedComponents(binary_mask)
    # ----------------------------------------------------------
    # START REPLACE: build batches via grid-partition + merging
    # ----------------------------------------------------------
    B = config.desired_batch_size_px
    MIN_BATCH = B // 2
    ...
    good_sized: list[tuple[np.ndarray, np.ndarray]] = []
    large_blobs: list[tuple[np.ndarray, np.ndarray]] = []
    small_pool: list[tuple[np.ndarray, np.ndarray]] = []
    for lbl in range(1, num_labels):
        ...
        if N_blob == 0:
            continue
        if N_blob > B:
            large_blobs.append((xs, ys))
        elif N_blob < MIN_BATCH:
            small_pool.append((xs, ys))
        else:
            good_sized.append((xs, ys))
    for xs_blob, ys_blob in large_blobs:
        subbatches = split_blob_grid(xs_blob, ys_blob, B)
        for xs_sub, ys_sub in subbatches:
            good_sized.append((xs_sub, ys_sub))
    merged_smalls = merge_small_blobs(small_pool, MIN_BATCH)
    for xs_s, ys_s in merged_smalls:
        good_sized.append((xs_s, ys_s))
    batches: list[PixelBatch] = []
    for xs_batch, ys_batch in good_sized:
        center_x_px = int(xs_batch.mean())
        center_y_px = int(ys_batch.mean())
        centroid_mx = (center_x_px - img_width_px / 2) * pixel_to_meter_x
        centroid_my = (center_y_px - img_height_px / 2) * pixel_to_meter_y
        targets: list[PixelTarget] = []
        for x_px, y_px in zip(xs_batch, ys_batch):
            meter_x = (x_px - img_width_px / 2) * pixel_to_meter_x
            meter_y = (y_px - img_height_px / 2) * pixel_to_meter_y
            point_index = int(y_px * img_width_px + x_px)
        ...
        batches.append(
            PixelBatch(
                center_pose=Pose(
                    pos=jnp.array([centroid_mx, centroid_my, 0.0]),
                    wxyz=config.design_pose.wxyz
                ),
    ...
    log.info(f"🔢 Design has {num_targets} targets in {num_batches} batches (grid-partition).")
    # ----------------------------------------------------------
    # END REPLACE
    # ----------------------------------------------------------
    ...
    with server.gui.add_folder("Session"):
        ...
        initial_target_max = len(batches[0].targets) - 1 if batches and batches[0].targets else 0
        target_index = server.gui.add_slider(
            "Target Index",
            min=0,
            max=initial_target_max,
            step=1,
            initial_value=0,
        )
    ...
    try:
        ...
        while True:
            ...
            if state_handle.value == "WORK":
                ...
                if target_index.value > target_index.max:
                    target_index.value = target_index.max
                ...
                pixel_radius_x = int(config.batch_radius_m / pixel_to_meter_x)
                pixel_radius_y = int(config.batch_radius_m / pixel_to_meter_y)
                cv2.ellipse(_img, (center_x_viz, center_y_viz), (pixel_radius_x, pixel_radius_y), 0, 0, 360, (0, 255, 0), 1)
                ...
                if target_index.value >= initial_target_max:
